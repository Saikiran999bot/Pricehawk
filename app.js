...
async function scrapeFlipkart(page) {
  ...
  // Wait for any heading element — broad selector survives redesigns
  try {
    await page.waitForSelector(
      '.B_NuCI, .yhB1nd, h1, [class*="title"], [class*="Title"]',
      { timeout: 15000 }
    );
  } catch (e) {
    // Continue even if selector times out — page might still have content
  }
  ...
  return page.evaluate(function() {
    ...
    // ── Title — wide selector list covering old and new Flipkart layouts ──
    var title = getText([
      '.B_NuCI', '.yhB1nd', 'h1.yhB1nd', 'h1._9E25nV',
      'h1', 'span.B_NuCI', '[class*="ProductTitle"]',
      'div[class*="title"] h1', 'div[class*="Title"] h1',
    ]);
    // Last resort: og:title meta
    if (!title) {
      var og = document.querySelector('meta[property="og:title"]');
      title = og ? og.getAttribute('content') : '';
    }
    title = title || '';

    // ── Price — covers multiple Flipkart layout versions ──────────────────
    var priceText = getText([
      '._30jeq3._16Jk6d', '._30jeq3', '._16Jk6d',
      'div._25b18c ._30jeq3', '[class*="price"] [class*="amount"]',
    ...
    var imageUrl = (imgEl && imgEl.src) || (ogImg && ogImg.getAttribute('content')) || '';
