...
// ─── Global scraper semaphore — 1 browser at a time (prevents OOM on free tier)
var scraperBusy   = false;
var scraperQueue  = [];

function acquireScraper() {
  return new Promise(function(resolve) {
    if (!scraperBusy) {
      scraperBusy = true;
      resolve();
    } else {
      scraperQueue.push(resolve);
    }
  });
}

function releaseScraper() {
  if (scraperQueue.length > 0) {
    var next = scraperQueue.shift();
    next();
  } else {
    scraperBusy = false;
  }
...
// ─── Amazon HTTP scrape (no browser) ─────────────────────────────────────────
async function scrapeAmazonHttp(url) {
  ...
  if (/api-services-support\.amazon|robot check|enter the characters/i.test(html)) {
    throw new Error('Amazon bot detection — falling back to browser');
  }
  ...
  for (var i = 0; i < jsonLdBlocks.length; i++) {
    try {
      var raw = jsonLdBlocks[i].replace(/<script[^>]*>/i, '').replace(/<\/script>/i, '');
      ...
      if (prod) {
        ...
        if (title && price > 0) {
          return {
            ...
            modelNumber:   prod.mpn || prod.sku || null,
            rating:        parseFloat((prod.aggregateRating && prod.aggregateRating.ratingValue) || 0) || null,
            inStock:       !(offer && offer.availability && offer.availability.includes('OutOfStock')),
  ...
  // ── LAYER 2: Amazon-specific HTML patterns ────────────────────────────────
  // Title
  var titleMatch = html.match(/id="productTitle"[^>]*>\s*([\s\S]*?)\s*<\/span>/) ||
                   html.match(/<title>([^|<]+)/);
  var title = titleMatch ? titleMatch[1].replace(/<[^>]+>/g, '').trim() : '';

  // Remove "Amazon.in: " prefix from page title
  title = title.replace(/^Amazon\.in\s*:\s*/i, '').trim();

  // Price — Amazon stores it as whole + fraction
  var priceWhole = (html.match(/class="a-price-whole">([^<]+)/) || [])[1];
  var priceFrac  = (html.match(/class="a-price-fraction">([^<]+)/) || [])[1] || '00';
  var price = 0;
  if (priceWhole) {
    price = parseFloat(priceWhole.replace(/[^0-9]/g,'') + '.' + priceFrac.replace(/[^0-9]/g,'')) || 0;
  }
  if (!price) {
    // Try priceblock patterns
    var priceMatch2 = html.match(/id="priceblock_ourprice"[^>]*>[\s\S]*?₹\s*([\d,]+)/) ||
                      html.match(/"price":"([\d.]+)"/) ||
                      html.match(/₹\s*([\d,]+)/);
    if (priceMatch2) price = parseFloat(priceMatch2[1].replace(/,/g,'')) || 0;
  }

  // Image
  var imgMatch = html.match(/id="landingImage"[^>]+src="([^"]+)"/) ||
                 html.match(/data-old-hires="([^"]+)"/) ||
                 html.match(/<meta property="og:image"\s+content="([^"]+)"/);
  var imageUrl = imgMatch ? imgMatch[1] : '';

  // Brand
  var brandMatch = html.match(/id="bylineInfo"[^>]*>[\s\S]*?(?:Visit the |Brand: )([^<]+)</i);
  var brand = brandMatch ? brandMatch[1].trim() : null;

  // og:title fallback
  if (!title) {
    var ogTitle = (html.match(/<meta property="og:title"\s+content="([^"]+)"/) || [])[1];
    title = ogTitle || '';
  }

  if (!title) throw new Error('Amazon HTTP: could not extract title');
  if (!imageUrl) {
    var ogImg = (html.match(/<meta property="og:image"\s+content="([^"]+)"/) || [])[1];
    imageUrl = ogImg || '';
  }

  return {
    title: title, price: price, originalPrice: null,
    imageUrl: imageUrl, brand: brand, modelNumber: null,
    rating: null, inStock: true, category: null,
  };
}

async function scrapeFlipkartHttp(url) {
  // Use a mobile app user-agent — less likely to be blocked than desktop browser
  var response = await axios.get(url, {
    timeout: 20000,
    maxRedirects: 10,
    headers: {
      // Mobile app UA — Flipkart serves cleaner responses to these
      ...
      'Sec-Fetch-Site': 'none',
      'Upgrade-Insecure-Requests': '1',
      'Referer': 'https://www.google.com/search?q=flipkart+product',
    },
  });
...
async function scrapeProduct(url, platform = 'other') {
  // Acquire semaphore — only 1 browser runs at a time to prevent OOM on free tier
  await acquireScraper();

  try {
    return await _doScrape(url, platform);
  } finally {
    releaseScraper();
  }
}
...
async function _doScrape(url, platform) {
  // Try HTTP-first for both Amazon and Flipkart — faster, no bot detection risk
  if (platform === 'amazon' || platform === 'flipkart') {
    try {
      var httpFn = platform === 'amazon' ? scrapeAmazonHttp : scrapeFlipkartHttp;
      console.log('[SCRAPER] Trying ' + platform + ' HTTP method first...');
      var result = await httpFn(url);
      if (result && result.title && result.price > 0) {
        console.log('[SCRAPER] ✅ HTTP method succeeded: "' + result.title.slice(0,50) + '" ₹' + result.price);
        return Object.assign({}, result, { resolvedUrl: url });
      }
    } catch (httpErr) {
  ...
  // Browser retries: 2 for Flipkart (saves RAM), 3 for others
  var maxAttempts = platform === 'flipkart' ? 2 : 3;
...
app.use(function(err, req, res, next) {
  console.error('[SERVER] Error on', req.path, ':', err.message);
