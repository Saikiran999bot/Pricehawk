...
async function launchBrowser() {
  var executablePath = process.env.CHROME_EXECUTABLE_PATH || await chromium.executablePath();
...
╚══════════════════════════════════════════════════╝
  ...
  Supabase : ${process.env.SUPABASE_URL ? process.env.SUPABASE_URL.slice(0,50) + '...' : '❌ NOT SET — add SUPABASE_URL'}
  Gemini   : ${geminiModel ? '✅ Connected (gemini-2.5-flash)' : '⚠️  Not set — math engine active'}
  Email    : ${isBrevoConfigured() ? '✅ Brevo ready — ' + process.env.BREVO_SENDER_EMAIL : '❌ NOT SET — add BREVO_API_KEY + BREVO_SENDER_EMAIL'}
  Chrome   : @sparticuz/chromium-min (Render-compatible)
